...
/* Returns the single instance of FreeType */
...
/* Generate the display list for a single character */
void FreeType::MakeDisplayList(FT_Face face, char ch, GLuint listBase, GLuint *texBase, unsigned short *widths)
{
    int i, j;
    ...
    glTranslatef(0, (float)(bitmapGlyph->top - bitmap.rows), 0);
...
/* Prints a rotated string */
void FreeType::PrintRotate(int index, int width, float x, float y, float xrot, float yrot, float zrot, const char *fmt, ...)
{
    vector<string> lines;
    ...
    // Split text into lines - shamelessly copied from NeHe's code :)
    start_line = text;
    const char *c;
    for (c = text; *c; c++)
	{
