...
#ifdef USE_FOPEN_S
  if (fopen_s(&m_stream, path, mode) != 0)
#else
    if ((m_stream = fopen(path, mode)) == NULL)
#endif
      throw std::runtime_error("Failed to open: " + string(path));
}
...
#else
	snprintf(path, MAX_RES_PATH, "%s.%s", base, ext);
#endif
